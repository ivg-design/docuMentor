...
class TUIAdapterClass extends EventEmitter {
  ...
  start(projectPath: string): void {
    ...
    this.lockFileManager = LockFileManager.getInstance(projectPath)
    ...
    // Send initial file progress (0/0) without updating lock file
    this.fileProgress = { processed: 0, total: 0 }
    this.send({
      type: 'file',
      files: this.fileProgress
    })
    
    // Start monitoring lock file (will send data when lock file exists)
    ...
    const sendLockInfo = async () => {
      try {
        ...
        if (lockInfo) {
          ...
          // Send default lock info when file doesn't exist yet
          this.send({
            type: 'lockInfo',
            lockInfo: {
              status: 'unlocked',
        ...
        // Ignore errors when lock file doesn't exist yet
      }
    }
