...
export class ObsidianTagOptimizer {
  ...
  public async optimizeTags(
    ...
    Logger.info('Starting AI-driven tag optimization...')
  ...
  private async generateOptimizationStrategy(
    ...
    Logger.info('Generating AI-driven optimization strategy...')
    ...
    for (const [groupType, groupDocs] of documentGroups) {
      Logger.info(`Optimizing tags for ${groupType} documents (${groupDocs.length} docs)...`)
  ...
  private async optimizeTagsForGroup(
    ...
    try {
      ...
      Logger.warning('AI optimization failed for group, using fallback')
...
Provide optimized tags for each document with reasoning.`
  ...
  private parseOptimizationResponse(
    ...
    try {
      ...
      Logger.warning('Error parsing AI response, using fallback')
    }
  ...
  private async applyOptimizations(
    ...
    for (const doc of documents) {
      ...
      if (optimization) {
        ...
        Logger.info(`Applied tag optimization to "${doc.title}": ${optimization.changes.length} changes`)
      } else {
  ...
  public async generateTagHierarchyDocument(
    ...
    Logger.info('Generating TAG_HIERARCHY.md...')
