...
class ConfigManager {
  ...
  private docuMentorRoot: string = resolve(__dirname, '../../../')

  constructor(configPath?: string) {
    // First look for config in docuMentor root, then fallback to user home
    if (!configPath) {
      const rootConfig = resolve(this.docuMentorRoot, '.documentor.config.json')
      const userConfig = resolve(homedir(), '.documentor', 'config.json')
      
      // Use docuMentor root config if it exists, otherwise use user config
      this.configPath = existsSync(rootConfig) ? rootConfig : userConfig
    } else {
