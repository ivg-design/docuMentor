...
type TUI struct {
	...
	workerPanel   *WorkerPanel  // NEW: Worker visualization panel
	...
	nodeCmd       *exec.Cmd     // The Node.js child process
...
func NewTUI() *TUI {
	...
	tui.workerPanel = NewWorkerPanel(tui.app)
	...
	tui.mainLayout = tview.NewFlex().SetDirection(tview.FlexRow).
		...
		AddItem(tui.workerPanel, 5, 0, false).   // 4. Worker visualization panel (NEW)
		AddItem(tui.pages, 0, 1, true).          // 5. Main logs area
...
// parseWorkerData parses worker data from JSON
func parseWorkerData(data map[string]interface{}) WorkerData {
	worker := WorkerData{}
	
	if id, ok := data["workerId"].(float64); ok {
	...
	if progress, ok := data["progress"].(float64); ok {
		worker.Progress = int(progress)
	}
	if elapsed, ok := data["timeElapsed"].(float64); ok {
		worker.TimeElapsed = int(elapsed)
	}
	...
	if stats, ok := data["stats"].(map[string]interface{}); ok {
		if completed, ok := stats["completed"].(float64); ok {
			worker.Stats.Completed = int(completed)
		}
		if failed, ok := stats["failed"].(float64); ok {
			worker.Stats.Failed = int(failed)
		}
		if avgTime, ok := stats["avgTime"].(float64); ok {
			worker.Stats.AvgTime = int(avgTime)
		}
		if totalTime, ok := stats["totalTime"].(float64); ok {
			worker.Stats.TotalTime = int(totalTime)
		}
	}
...
func (t *TUI) handleMessage(msg Message) {
	t.app.QueueUpdateDraw(func() {
		...
		case "file":
			...
			t.updateInfoBox()
		case "worker":
			// Handle single worker update
			if data, ok := msg.Data.(map[string]interface{}); ok {
				worker := parseWorkerData(data)
				t.workerPanel.UpdateWorker(worker)
			}
		case "worker_batch":
			// Handle batch worker update
			if data, ok := msg.Data.(map[string]interface{}); ok {
				if workers, ok := data["workers"].([]interface{}); ok {
					for _, w := range workers {
						if workerData, ok := w.(map[string]interface{}); ok {
							worker := parseWorkerData(workerData)
							t.workerPanel.UpdateWorker(worker)
						}
					}
				}
...
func (t *TUI) Run() error {
	...
	defer t.CleanupDocumentor()
	
	// Run the app (no stdin reading needed, we get data from child process)
	return t.app.Run()
}
...
func main() {
	// Check for ULTRA test mode
	if len(os.Args) >= 2 && os.Args[1] == "--ultra" {
		RunProperUltraTest()
		return
	}
	
	// Check for old test mode
	if len(os.Args) >= 2 && os.Args[1] == "--test" {
		RunTestTUI()
		return
	}
	
	// Get command line arguments (pass through to Node.js)
	args := os.Args[1:]
	
	// Create TUI
	tui := NewTUI()
	
	// Launch Node.js documentor as child process
	go func() {
		if err := tui.LaunchDocumentor(args); err != nil {
			fmt.Fprintf(os.Stderr, "Failed to launch documentor: %v\n", err)
			os.Exit(1)
		}
	}()
