...
export class ObsidianIntegration {
  ...
  public async processDocuments(sourceFiles: string[]): Promise<ProcessedDocument[]> {
    ...
    console.log(`Starting Obsidian integration for ${sourceFiles.length} files...`)
    ...
    try {
      ...
      console.log(`Obsidian integration completed in ${this.context.processingStats.processingTimeMs}ms`)

      return finalDocuments
    } catch (error) {
      console.error('Error during Obsidian integration:', error)
      throw error
    }
  }
  ...
  private async initializeProcessing(sourceFiles: string[]): Promise<void> {
    console.log('Initializing Obsidian processing...')
    ...
    try {
      ...
      for (const file of files) {
        try {
          ...
          console.warn(`Warning: Could not parse existing document ${file}:`, error)
        }
      }

      console.log(`Scanned ${this.context.existingDocs.size} existing documents`)
    } catch (error) {
      console.log('No existing documents found or error scanning:', error)
    }
  }
  ...
  private async parseExistingDocument(filePath: string, content: string): Promise<ProcessedDocument | null> {
    try {
      ...
      console.warn(`Could not parse document ${filePath}:`, error)
      return null
    }
  }
  ...
  private async initializeTagHierarchy(): Promise<void> {
    ...
    console.log(`Initialized tag hierarchy with ${hierarchy.size} parent categories`)
  }
  ...
  private async generateBaseDocuments(sourceFiles: string[]): Promise<ProcessedDocument[]> {
    console.log('Generating base documentation...')
    const documents: ProcessedDocument[] = []

    for (let i = 0; i < sourceFiles.length; i++) {
      const sourceFile = sourceFiles[i]
      console.log(`Generating documentation for ${path.basename(sourceFile)} (${i + 1}/${sourceFiles.length})...`)
      ...
      try {
        ...
        console.error(`Error generating documentation for ${sourceFile}:`, error)
  ...
  private async processObsidianPipeline(documents: ProcessedDocument[]): Promise<ProcessedDocument[]> {
    console.log('Processing through Obsidian pipeline...')
  ...
  private async optimizeAndFinalize(documents: ProcessedDocument[]): Promise<ProcessedDocument[]> {
    console.log('Finalizing documents...')
  ...
  private async generateFinalContent(doc: ProcessedDocument): Promise<string> {
    ...
    // Add frontmatter
    const frontmatterYaml = this.frontmatterGenerator.generateYaml(doc.frontmatter)
    parts.push('---', frontmatterYaml, '---', '')
  ...
  private async writeDocument(doc: ProcessedDocument): Promise<void> {
    try {
      ...
      console.error(`Error writing document ${doc.filePath}:`, error)
  ...
  private async writeTagHierarchyDocument(documents: ProcessedDocument[]): Promise<void> {
    ...
    try {
      ...
      console.log('Generated TAG_HIERARCHY.md')
    } catch (error) {
      console.error('Error writing TAG_HIERARCHY.md:', error)
    }
  }
  ...
  private async writeProcessingSummary(): Promise<void> {
    ...
    try {
      ...
      console.log('Generated PROCESSING_SUMMARY.md')
    } catch (error) {
      console.error('Error writing PROCESSING_SUMMARY.md:', error)
    }
  }
