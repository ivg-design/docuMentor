...
export class ProjectTypeDetector {
  ...
  async detect(projectPath: string): Promise<DetectionResult> {
    ...
    // Use AI for intelligent detection instead of heuristics
    const aiResult = await this.detectWithAI(projectPath, indicators)

    // If AI detection fails, fall back to basic heuristics
    if (!aiResult) {
...
6. Overall project architecture`
  ...
  private async gatherIndicators(projectPath: string): Promise<ProjectIndicators> {
    ...
    try {
      ...
      console.warn(`Warning: Could not fully analyze project at ${projectPath}: ${error}`)
    }
